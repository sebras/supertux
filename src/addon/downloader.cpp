...
{
size_t
my_curl_string_append(void* ptr, size_t size, size_t nmemb, void* userdata)
{
...
size_t
my_curl_physfs_write(void* ptr, size_t size, size_t nmemb, void* userdata)
{
...
class Transfer
{
 ...
 public:
        ...
        m_fout(PHYSFS_openWrite(outfile.c_str()), PHYSFS_close)
  {
    ...
    else
    {
      ...
      curl_easy_setopt(m_handle, CURLOPT_USERAGENT,
                       "SuperTux/" PACKAGE_VERSION " libcURL");

      curl_easy_setopt(m_handle, CURLOPT_WRITEDATA, this);
      curl_easy_setopt(m_handle, CURLOPT_WRITEFUNCTION,
                       &Transfer::on_data_wrap);
      ...
      curl_easy_setopt(m_handle, CURLOPT_PROGRESSFUNCTION,
                       &Transfer::on_progress_wrap);
    }
  }

  ~Transfer() { curl_easy_cleanup(m_handle); }
  ...
  size_t
  on_data(void* ptr, size_t size, size_t nmemb)
  {
  ...
  int
  on_progress(double dltotal, double dlnow, double ultotal, double ulnow)
  {
 ...
 private:
  static size_t
  on_data_wrap(char* ptr, size_t size, size_t nmemb, void* userdata)
  {
    return static_cast<Transfer*>(userdata)->on_data(ptr, size, nmemb);
  }

  static int
  on_progress_wrap(void* userdata, double dltotal, double dlnow, double ultotal,
                   double ulnow)
  {
    return static_cast<Transfer*>(userdata)
        ->on_progress(dltotal, dlnow, ultotal, ulnow);
  }
...
Downloader::Downloader()
    : m_multi_handle(), m_transfers(), m_next_transfer_id(1)
{
...
Downloader::download(const std::string& url, const std::string& filename)
{
  ...
  std::unique_ptr<PHYSFS_file, int (*)(PHYSFS_File*)> fout(
      PHYSFS_openWrite(filename.c_str()), PHYSFS_close);
...
Downloader::abort(TransferId id)
{
  ...
  else
  {
    ...
    for (auto& callback : status->callbacks)
    {
      ...
      {
        log_warning << "Illegal exception in Downloader: " << err.what()
                    << std::endl;
      }
    }
  }
...
Downloader::update()
{
  ...
  CURLMcode ret;
  int running_handles;
  while ((ret = curl_multi_perform(m_multi_handle, &running_handles)) ==
  ...
  while ((msg = curl_multi_info_read(m_multi_handle, &msgs_in_queue)))
  {
    switch (msg->msg)
    {
      case CURLMSG_DONE:
      {
        ...
        auto it =
            std::find_if(m_transfers.begin(), m_transfers.end(),
        ...
        if (msg->data.result == CURLE_OK)
        {
          ...
          for (auto& callback : status->callbacks)
          {
            ...
            {
              success = false;
              log_warning << "Exception in Downloader: " << err.what()
                          << std::endl;
        ...
        {
          log_warning << "Error: " << curl_easy_strerror(msg->data.result)
                      << std::endl;
          for (auto& callback : status->callbacks)
          {
            try
...
Downloader::request_download(const std::string& url, const std::string& outfile)
{
  ...
  std::unique_ptr<Transfer> transfer(
      new Transfer(*this, m_next_transfer_id++, url, outfile));
