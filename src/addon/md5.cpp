...
MD5::MD5() : buffer(), digest(), finalized() { init(); }

void
MD5::update(uint8_t* input, uint32_t input_length)
{
  ...
  if ((count[0] += ((uint32_t)input_length << 3)) <
      ((uint32_t)input_length << 3))
    count[1]++;
...
void
MD5::update(std::istream& stream)
{
  uint8_t buffer_[1024];

  while (stream.good())
  {
    stream.read((char*)buffer_,
                1024);  // note that return value of read is unusable.
...
void
MD5::update(std::ifstream& stream)
{
  uint8_t buffer_[1024];
...
MD5::MD5(std::istream& stream) : finalized()
{
...
MD5::MD5(std::ifstream& stream) : finalized()
{
  init();  // must called by all constructors
  update(stream);
  finalize();
...
void
MD5::transform(uint8_t block[64])
{
...
void
MD5::encode(uint8_t* output, uint32_t* input, uint32_t len)
{
  unsigned int i, j;
...
void
MD5::decode(uint32_t* output, uint8_t* input, uint32_t len)
{
  unsigned int i, j;
...
void
MD5::memcpy(uint8_t* output, uint8_t* input, uint32_t len)
{
  unsigned int i;
...
void
MD5::memset(uint8_t* output, uint8_t value, uint32_t len)
{
  unsigned int i;
...
inline unsigned int
MD5::rotate_left(uint32_t x, uint32_t n)
{
  return (x << n) | (x >> (32 - n));
}
...
inline void
MD5::FF(uint32_t& a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, uint32_t s,
        uint32_t ac)
{
  a += F(b, c, d) + x + ac;
  a = rotate_left(a, s) + b;
...
inline void
MD5::GG(uint32_t& a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, uint32_t s,
        uint32_t ac)
{
  a += G(b, c, d) + x + ac;
  a = rotate_left(a, s) + b;
...
inline void
MD5::HH(uint32_t& a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, uint32_t s,
        uint32_t ac)
{
  a += H(b, c, d) + x + ac;
  a = rotate_left(a, s) + b;
...
inline void
MD5::II(uint32_t& a, uint32_t b, uint32_t c, uint32_t d, uint32_t x, uint32_t s,
        uint32_t ac)
{
  a += I(b, c, d) + x + ac;
  a = rotate_left(a, s) + b;
