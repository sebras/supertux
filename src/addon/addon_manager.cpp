...
MD5
md5_from_file(const std::string& filename)
{
  // TODO: this does not work as expected for some files -- IFileStream seems to
  // not always behave like an ifstream.
...
static void
add_to_dictionary_path(void* data, const char* origdir, const char* fname)
{
  std::string full_path = std::string(origdir) + "/" + std::string(fname);
  if (PHYSFS_isDirectory(full_path.c_str()))
  {
    log_debug << "Adding \"" << full_path << "\" to dictionary search path"
              << std::endl;
...
static void
remove_from_dictionary_path(void* data, const char* origdir, const char* fname)
{
  std::string full_path = std::string(origdir) + "/" + std::string(fname);
  if (PHYSFS_isDirectory(full_path.c_str()))
  {
...
AddonManager::AddonManager(const std::string& addon_directory,
    ...
    : m_downloader(),
      ...
      m_repository_url(
          "https://raw.githubusercontent.com/SuperTux/addons/master/"
          "index-0_4_0.nfo"),
      ...
      m_transfer_status()
{
  ...
  for (auto& addon : m_addon_config)
  {
    if (addon.enabled)
    {
      ...
      {
        log_warning << "failed to enable addon from config: " << err.what()
                    << std::endl;
      }
    }
  }
  ...
  if (PHYSFS_exists(ADDON_INFO_PATH))
  {
    ...
    {
      log_warning << "parsing repository.nfo failed: " << err.what()
                  << std::endl;
    }
  }
  else
...
AddonManager::request_check_online()
{
  if (m_transfer_status)
  {
    throw std::runtime_error(
        "only async request can be made to AddonManager at a time");
  }
  else
  {
    m_transfer_status =
        m_downloader.request_download(m_repository_url, ADDON_INFO_PATH);
    ...
    m_transfer_status->then([this](bool success)
                            {
                              ...
                              {
                                m_repository_addons =
                                    parse_addon_infos(ADDON_INFO_PATH);
...
AddonManager::request_install_addon(const AddonId& addon_id)
{
  ...
  {
    throw std::runtime_error(
        "only one addon install request allowed at a time");
  }
  else
  {
    {  // remove addon if it already exists
      auto it =
          std::find_if(m_installed_addons.begin(), m_installed_addons.end(),
    ...
    std::string install_filename =
        FileSystem::join(m_addon_directory, addon.get_filename());

    m_transfer_status =
        m_downloader.request_download(addon.get_url(), install_filename);
        ...
        [this, install_filename, addon_id](bool success)
        {
          ...
          if (success)
          {
            ...
            if (repository_addon.get_md5() != md5.hex_digest())
            {
              ...
              {
                log_warning << "PHYSFS_delete failed: " << PHYSFS_getLastError()
                            << std::endl;
              }

              throw std::runtime_error(
                  "Downloading Add-on failed: MD5 checksums differ");
            }
            else
            {
              ...
              if (!realdir)
              {
                throw std::runtime_error("PHYSFS_getRealDir failed: " +
                                         install_filename);
              }
              else
              {
...
AddonManager::install_addon(const AddonId& addon_id)
{
  ...
  std::string install_filename =
      FileSystem::join(m_addon_directory, repository_addon.get_filename());
...
AddonManager::uninstall_addon(const AddonId& addon_id)
{
  ...
  log_debug << "deleting file \"" << addon.get_install_filename() << "\""
            << std::endl;
  PHYSFS_delete(addon.get_install_filename().c_str());
  m_installed_addons.erase(
      std::remove_if(m_installed_addons.begin(), m_installed_addons.end(),
...
AddonManager::enable_addon(const AddonId& addon_id)
{
  ...
  {
    log_debug << "Adding archive \"" << addon.get_install_filename()
              << "\" to search path" << std::endl;
    ...
    {
      log_warning << "Could not add " << addon.get_install_filename()
                  << " to search path: " << PHYSFS_getLastError() << std::endl;
    }
    else
    {
      if (addon.get_type() == Addon::LANGUAGEPACK)
      {
        PHYSFS_enumerateFilesCallback(addon.get_id().c_str(),
                                      add_to_dictionary_path, NULL);
      }
...
AddonManager::disable_addon(const AddonId& addon_id)
{
  ...
  {
    log_debug << "Removing archive \"" << addon.get_install_filename()
              << "\" from search path" << std::endl;
    if (PHYSFS_removeFromSearchPath(addon.get_install_filename().c_str()) == 0)
    {
      log_warning << "Could not remove " << addon.get_install_filename()
                  << " from search path: " << PHYSFS_getLastError()
                  << std::endl;
    }
    else
    {
      ...
      {
        PHYSFS_enumerateFilesCallback(addon.get_id().c_str(),
                                      remove_from_dictionary_path, NULL);
      }
...
AddonManager::scan_for_archives() const
{
  ...
  std::unique_ptr<char*, decltype(&PHYSFS_freeList)> rc(
      PHYSFS_enumerateFiles(m_addon_directory.c_str()), PHYSFS_freeList);
...
{
  std::unique_ptr<char*, decltype(&PHYSFS_freeList)> rc2(
      PHYSFS_enumerateFiles("/"), PHYSFS_freeList);
  for (char** j = rc2.get(); *j != 0; ++j)
  {
    if (has_suffix(*j, ".nfo"))
    {
      ...
      if (!realdir)
      {
        log_warning << "PHYSFS_getRealDir() failed for " << nfo_filename << ": "
                                    ...
                                    const std::string& md5)
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        log_warning << "Could not load add-on info for " << archive << ": "
                    << e.what() << std::endl;
      }
    }
  ...
  try
  {
    ...
    else
    {
      ...
      for (auto const& addon_node : addon_collection.get_objects())
      {
        ...
        {
          log_warning << "Unknown token '" << addon_node.get_name()
                      << "' in Add-on list" << std::endl;
        }
        else
        {
...
AddonManager::check_for_langpack_updates()
{
  ...
  try
  {
    ...
    try
    {
      ...
      log_debug << "Looking for language addon with ID " << addon_id << "..."
                << std::endl;
      ...
      try
      {
        ...
        {
          log_debug << "Language addon " << addon_id
                    << " is already the latest version." << std::endl;
          return;
        }
