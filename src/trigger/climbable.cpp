...
Climbable::Climbable(const ReaderMapping& reader)
    : climbed_by(0), activate_try_timer(), message()
{
...
Climbable::draw(DrawingContext& context)
{
  if (climbed_by && !message.empty())
  {
    ...
    Vector pos =
        Vector(0, SCREEN_HEIGHT / 2 - Resources::normal_font->get_height() / 2);
    context.draw_center_text(Resources::normal_font, _(message), pos, LAYER_HUD,
                             Climbable::text_color);
  ...
  if ((type == EVENT_ACTIVATE) || (activate_try_timer.started()))
  {
    if (player.get_grabbed_object() == NULL)
    {
      ...
      else
      {
        ...
        if (player.get_bbox().p1.x < bbox.p1.x - GRACE_DX)
          player.add_velocity(Vector(POSITION_FIX_AX, -13));
        if (player.get_bbox().p2.x > bbox.p2.x + GRACE_DX)
          player.add_velocity(Vector(-POSITION_FIX_AX, -13));
        if (player.get_bbox().p1.y < bbox.p1.y - GRACE_DY)
          player.add_velocity(Vector(0, POSITION_FIX_AY));
        if (player.get_bbox().p2.y > bbox.p2.y + GRACE_DY)
          player.add_velocity(Vector(0, -POSITION_FIX_AY));
      }
    }
  }
