...
SecretAreaTrigger::SecretAreaTrigger(const ReaderMapping& reader)
    : message_timer(), message_displayed(), message(), fade_tilemap(), script()
{
...
SecretAreaTrigger::draw(DrawingContext& context)
{
  if (message_timer.started())
  {
    ...
    context.draw_center_text(Resources::normal_font, message, pos, LAYER_HUD,
                             SecretAreaTrigger::text_color);
