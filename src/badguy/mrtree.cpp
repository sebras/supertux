...
MrTree::MrTree(const ReaderMapping& reader)
    : WalkingBadguy(reader, "images/creatures/mr_tree/mr_tree.sprite", "left",
                    "right")
{
...
MrTree::collision_squished(GameObject& object)
{
  ...
  for (int px = (int)stumpy->get_bbox().p1.x; px < (int)stumpy->get_bbox().p2.x;
       px += 10)
  {
    ...
    Sector::current()->add_object(std::make_shared<SpriteParticle>(
        "images/objects/particles/leaf.sprite", "default", ppos, ANCHOR_MIDDLE,
        pspeed, paccel, LAYER_OBJECTS - 1));
  }

  if (!frozen)
  {  // Frozen Mr.Trees don't spawn any PoisonIvys.
    // spawn PoisonIvy
    ...
    // spawn PoisonIvy
    Vector leaf2_pos(stumpy_pos.x + sprite->get_current_hitbox_width() + 1,
                     stumpy_pos.y - POISONIVY_Y_OFFSET);
