...
Igel::Igel(const Vector& pos, Direction d)
    : WalkingBadguy(pos, d, "images/creatures/igel/igel.sprite", "walking-left",
                    "walking-right"),
      turn_recover_timer()
{
  walk_speed = IGEL_SPEED;
...
Igel::can_see(const MovingObject& o)
{
  ...
  bool inReach_left =
      ((ob.p2.x < bbox.p1.x) &&
       (ob.p2.x >= bbox.p1.x - ((dir == LEFT) ? RANGE_OF_VISION : 0)));
  bool inReach_right =
      ((ob.p1.x > bbox.p2.x) &&
       (ob.p1.x <= bbox.p2.x + ((dir == RIGHT) ? RANGE_OF_VISION : 0)));
...
Igel::collision_bullet(Bullet& bullet, const CollisionHit& hit)
{
  ...
  if (((dir == LEFT) && hit.left) || ((dir == RIGHT) && hit.right) ||
      ...
      ((bullet.get_type() == FIRE_BONUS) && (frozen)))
  {
