...
Haywire::Haywire(const ReaderMapping& reader)
    : WalkingBadguy(reader, "images/creatures/haywire/haywire.sprite", "left",
                    "right"),
      is_exploding(false),
...
Haywire::collision_squished(GameObject& object)
{
  ...
  if (!is_exploding)
  {
    set_action((dir == LEFT) ? "ticking-left" : "ticking-right",
               /* loops = */ -1);
    ...
    ticking->play();
    grunting =
        SoundManager::current()->create_sound_source("sounds/grunts.ogg");
