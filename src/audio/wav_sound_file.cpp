...
read32LE(PHYSFS_file* file)
{
  uint32_t result;
  if (PHYSFS_readULE32(file, &result) == 0) throw SoundError("file too short");

  return result;
}

static inline uint16_t
read16LE(PHYSFS_file* file)
{
  uint16_t result;
  if (PHYSFS_readULE16(file, &result) == 0) throw SoundError("file too short");

  return result;
}

WavSoundFile::WavSoundFile(PHYSFS_file* file_) : file(file_), datastart()
{
  assert(file);
  char magic[4];
  ...
  do
  {
    ...
    if (strncmp(chunkmagic, "fmt ", 4) == 0) break;
  ...
  if (chunklen < 16) throw SoundError("Format chunk too short");

  // parse format
  uint16_t encoding = read16LE(file);
  if (encoding != 1) throw SoundError("only PCM encoding supported");
  ...
  do
  {
    ...
    if (strncmp(chunkmagic, "data", 4) == 0) break;
...
WavSoundFile::~WavSoundFile() { PHYSFS_close(file); }
...
WavSoundFile::read(void* buffer, size_t buffer_size)
{
  ...
  if (bits_per_sample != 16) return readsize;
