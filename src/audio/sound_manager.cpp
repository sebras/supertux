...
SoundManager::intern_create_sound_source(const std::string& filename)
{
  ...
  else
  {
    ...
    log_debug << "Uncached sound \"" << filename << "\" requested to be played"
              << std::endl;
  }
...
{
  if (!sound_enabled) return create_dummy_sound_source();
...
SoundManager::manage_source(std::unique_ptr<SoundSource> source)
{
  ...
  {
    std::unique_ptr<OpenALSoundSource> openal_source(
        dynamic_cast<OpenALSoundSource*>(source.release()));
...
{
  if (filename == current_music && music_source != NULL) return;
  ...
  try
  {
    ...
    if (fade) newmusic->set_fading(StreamSoundSource::FadingOn, .5f);
    newmusic->play();
  ...
  catch (std::exception& e)
  {
    log_warning << "Couldn't play music file '" << filename << "': " << e.what()
                << std::endl;
    // When this happens, previous music continued playing, stop it, just in
    // case.
    stop_music(0);
  }
}
...
SoundManager::set_listener_position(const Vector& pos)
{
  ...
  if (current_ticks - lastticks < 300) return;
