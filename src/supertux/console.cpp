...
ConsoleBuffer::ConsoleBuffer() : m_lines(), m_console(nullptr) {}
  ...
  if (m_vm == NULL)
  {
    ...
    if (new_vm == NULL)
      throw scripting::SquirrelError(
          m_vm, "Couldn't create new VM thread for console");

    // store reference to thread
    sq_resetobject(&m_vm_object);
    if (SQ_FAILED(sq_getstackobj(m_vm, -1, &m_vm_object)))
      throw scripting::SquirrelError(m_vm,
                                     "Couldn't get vm object for console");
    ...
    if (SQ_FAILED(sq_setdelegate(new_vm, -2)))
      throw scripting::SquirrelError(new_vm,
                                     "Couldn't set console_table delegate");
...
namespace
{
void sq_insert_commands(std::list<std::string>& cmds, HSQUIRRELVM vm,
                        std::string table_prefix, std::string search_prefix);

/**
 * Acts upon key,value on top of stack:
 * Appends key (plus type-dependent suffix) to cmds if table_prefix+key starts
 * with search_prefix;
 * Calls sq_insert_commands if search_prefix starts with table_prefix+key (and
 * value is a table/class/instance);
 */
void
sq_insert_command(std::list<std::string>& cmds, HSQUIRRELVM vm,
                  std::string table_prefix, std::string search_prefix)
{
 ...
 */
void
sq_insert_commands(std::list<std::string>& cmds, HSQUIRRELVM vm,
                   std::string table_prefix, std::string search_prefix)
{
...
{
  // int autocompleteFrom = m_inputBuffer.find_last_of(" ();+",
  // m_inputBufferPosition);
  int autocompleteFrom = m_inputBuffer.find_last_not_of(
  ...
  std::string prefix = m_inputBuffer.substr(
      autocompleteFrom, m_inputBufferPosition - autocompleteFrom);
  ...
  if (cmds.size() > 1)
  {
    // multiple matches: show all matches and set input buffer to longest common
    // prefix
    ...
    while (cmds.begin() != cmds.end())
    {
      ...
      {
        if (cmd.compare(0, n, commonPrefix) != 0)
          commonPrefix.resize(n - 1);
        else
          break;
      }
    }
...
Console::show()
{
  if (!g_config->developer_mode) return;

  m_focused = true;
  m_height = 256;
  m_alpha = 1.0;
  //  SDL_EnableKeyRepeat(SDL_DEFAULT_REPEAT_DELAY,
  //  SDL_DEFAULT_REPEAT_INTERVAL); // Useless in SDL2 :  if you want to disable
  //  repeat, then you need to check if the key was repeated and ignore it.
}

void
Console::open()
{
  if (m_stayOpen < 2) m_stayOpen += 1.5;
}
...
Console::update(float elapsed_time)
{
  ...
  if (m_backgroundOffset > (int)m_background->get_width())
    m_backgroundOffset -= (int)m_background->get_width();
}
...
Console::draw(DrawingContext& context) const
{
  ...
  context.draw_surface(
      m_background2, Vector(SCREEN_WIDTH / 2 - m_background->get_width() / 2 -
                                m_background->get_width() + m_backgroundOffset,
                            m_height - m_background->get_height()),
      layer);
  context.draw_surface(m_background2,
                       Vector(SCREEN_WIDTH / 2 - m_background->get_width() / 2 +
                                  m_backgroundOffset,
                              m_height - m_background->get_height()),
                       layer);
  for (int x = (SCREEN_WIDTH / 2 - m_background->get_width() / 2 -
                (static_cast<int>(ceilf((float)SCREEN_WIDTH /
                                        (float)m_background->get_width()) -
                                  1) *
                 m_background->get_width()));
       x < SCREEN_WIDTH; x += m_background->get_width())
  {
    context.draw_surface(
        m_background, Vector(x, m_height - m_background->get_height()), layer);
  }
  ...
  if (m_focused)
  {
    ...
    context.draw_text(m_font, "> " + m_inputBuffer, Vector(4, py), ALIGN_LEFT,
                      layer);
    if (SDL_GetTicks() % 1000 < 750)
    {
      ...
      context.draw_text(
          m_font, "_",
          Vector(4 + (cursor_px * m_font->get_text_width("X")), py), ALIGN_LEFT,
          layer);
    }
  }

  int skipLines = -m_offset;
  for (std::list<std::string>::iterator i = m_buffer.m_lines.begin();
       i != m_buffer.m_lines.end(); ++i)
  {
