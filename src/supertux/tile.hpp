...
class Tile
{
 public:
  ...
  enum
  {
    ...
    WORLDMAP_CNSEW =
        WORLDMAP_NORTH | WORLDMAP_SOUTH | WORLDMAP_EAST | WORLDMAP_WEST
  };
 ...
 public:
  ...
  Tile(const std::vector<ImageSpec>& images,
       const std::vector<ImageSpec>& editor_images, uint32_t attributes,
       uint32_t data, float fps, std::string obj_name = "",
       std::string obj_data = "");
  ~Tile();
  ...
  bool is_solid(const Rectf& tile_bbox, const Rectf& position,
   ...
   * tile don't matter.*/
  bool is_collisionful(const Rectf& tile_bbox, const Rectf& position,
                       const Vector& movement) const;
