...
void
wait(HSQUIRRELVM vm, float seconds)
{
  TimeScheduler::instance->schedule_thread(vm, game_time + seconds);
}

void
wait_for_screenswitch(HSQUIRRELVM vm)
{
...
void
shrink_screen(float dest_x, float dest_y, float seconds)
{
  if (g_config->transitions_enabled)
  {
...
void
load_worldmap(const std::string& filename)
{
  ...
  else
  {
    ScreenManager::current()->push_screen(std::unique_ptr<Screen>(
        new WorldMap(filename, WorldMap::current()->get_savegame())));
  }
}

void
load_level(const std::string& filename)
{
  ...
  {
    ScreenManager::current()->push_screen(std::unique_ptr<Screen>(
        new GameSession(filename, GameSession::current()->get_savegame())));
  }
}

void
import(HSQUIRRELVM vm, const std::string& filename)
{
  IFileStream in(filename);

  if (SQ_FAILED(
          sq_compile(vm, squirrel_read_char, &in, filename.c_str(), SQTrue)))
...
void
debug_draw_solids_only(bool enable)
{
  Sector::draw_solids_only = enable;
}

void
debug_draw_editor_images(bool enable)
{
  Tile::draw_editor_images = enable;
}

void
debug_worldmap_ghost(bool enable)
{
  using namespace worldmap;
...
void
play_music(const std::string& filename)
{
  SoundManager::current()->play_music(filename);
}

void
play_sound(const std::string& filename)
{
...
whereami()
{
  if (!validate_sector_player()) return;
  ::Player* tux = Sector::current()->player;
  log_info << "You are at x " << ((int)tux->get_pos().x) << ", y "
           << ((int)tux->get_pos().y) << std::endl;
}
...
gotoend()
{
  if (!validate_sector_player()) return;
  ::Player* tux = Sector::current()->player;
  tux->move(Vector((Sector::current()->get_width()) - (SCREEN_WIDTH * 2), 0));
  Sector::current()->camera->reset(Vector(tux->get_pos().x, tux->get_pos().y));
}

void
warp(float offset_x, float offset_y)
{
  if (!validate_sector_player()) return;
  ::Player* tux = Sector::current()->player;
...
{
  if (!validate_sector_player()) return;
  log_info << "Camera is at " << Sector::current()->camera->get_translation().x
...
void
set_game_speed(float speed)
{
  ::g_game_speed = speed;
}

void
record_demo(const std::string& filename)
{
  if (GameSession::current() == 0)
  {
    log_info << "No game session" << std::endl;
...
void
play_demo(const std::string& filename)
{
  if (GameSession::current() == 0)
  {
    log_info << "No game session" << std::endl;
  ...
  g_config->random_seed =
      GameSession::current()->get_demo_random_seed(filename);
