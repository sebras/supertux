...
SpriteParticle::SpriteParticle(std::string sprite_name, std::string action,
                               Vector position_, AnchorPoint anchor,
                               Vector velocity_, Vector acceleration_,
                               int drawing_layer_)
    : sprite(),
      position(position_),
      velocity(velocity_),
      ...
      glow(false)
{
  ...
  if (!sprite.get())
    throw std::runtime_error("Could not load sprite " + sprite_name);
...
SpriteParticle::update(float elapsed_time)
{
  ...
  if ((position.x < camera.x - 128) ||
      (position.x > SCREEN_WIDTH + camera.x + 128) ||
      (position.y < camera.y - 128) ||
      (position.y > SCREEN_HEIGHT + camera.y + 128))
  {
    remove_me();
    return;
  }
