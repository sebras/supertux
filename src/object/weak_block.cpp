...
WeakBlock::WeakBlock(const ReaderMapping& lisp)
    : MovingSprite(lisp, "images/objects/weak_block/strawbox.sprite",
...
WeakBlock::collision(GameObject& other, const CollisionHit& hit)
{
  switch (state)
  {
    ...
    case STATE_BURNING:
      if (sprite_name != "images/objects/weak_block/strawbox.sprite") break;
  ...
  switch (state)
  {
    ...
    case STATE_BURNING:
      ...
      if (linked)
      {
        ...
        {
          lightsprite->set_color(Color(0.2f + gameRandom.rand(20) / 100.0f,
