...
Background::draw_image(DrawingContext& context, const Vector& pos_)
{
  ...
  int start_x = static_cast<int>(
      floorf((cliprect.get_left() - (pos_.x - image->get_width() / 2.0f)) /
             image->get_width()));
  int end_x = static_cast<int>(ceilf((cliprect.get_right() -
                                      (pos_.x + image->get_width() / 2.0f)) /
                                     image->get_width())) +
              1;
  int start_y = static_cast<int>(
      floorf((cliprect.get_top() - (pos_.y - image->get_height() / 2.0f)) /
             image->get_height()));
  int end_y = static_cast<int>(ceilf((cliprect.get_bottom() -
                                      (pos_.y + image->get_height() / 2.0f)) /
                                     image->get_height())) +
              1;
  ...
  switch (alignment)
  {
    ...
    case BOTTOM_ALIGNMENT:
      ...
      {
        Vector p(
            pos_.x + x * image->get_width() - image->get_width() / 2.0f,
...
Background::draw(DrawingContext& context)
{
  ...
  Vector center_offset(
      context.get_translation().x - translation_range.width / 2.0f,
