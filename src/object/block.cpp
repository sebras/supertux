...
Block::collision(GameObject& other, const CollisionHit&)
{
  ...
  //   3) the object is being hit from below (baguys don't get killed for
  //   activating boxes)
  ...
  bool is_bomb =
      (bomb !=
       0);  // bombs need to explode, although they are considered portable
  bool hit_mo_from_below =
      ((moving_object == 0) || (moving_object->get_bbox().get_bottom() <
                                (bbox.get_top() + SHIFT_DELTA)));
...
Block::start_bounce(GameObject* hitter)
{
  ...
  if (hitter_mo)
  {
    ...
    float offset =
        (bbox.get_middle().x - center_of_hitter) * 2 / bbox.get_width();
...
Block::break_me()
{
  ...
  sector->add_object(std::make_shared<BrokenBrick>(sprite->clone(), get_pos(),
                                                   Vector(-100, -400)));
  sector->add_object(std::make_shared<BrokenBrick>(
      sprite->clone(), get_pos() + Vector(0, 16), Vector(-150, -300)));
  sector->add_object(std::make_shared<BrokenBrick>(
      sprite->clone(), get_pos() + Vector(16, 0), Vector(100, -400)));
  sector->add_object(std::make_shared<BrokenBrick>(
      sprite->clone(), get_pos() + Vector(16, 16), Vector(150, -300)));
  remove_me();
}
