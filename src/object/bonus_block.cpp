...
BonusBlock::BonusBlock(const Vector& pos, int data)
    : Block(SpriteManager::current()->create(
          "images/objects/bonus_block/bonusblock.sprite")),
      contents(),
      object(),
      hit_counter(1),
      sprite_name(),
      ...
      lightsprite()
{
    ...
    else
    {
      if (contents == CONTENT_CUSTOM)
      {
        ...
        GameObjectPtr game_object =
            ObjectFactory::instance().create(token, object_mapping);
      ...
      {
        log_warning << "Invalid element '" << token << "' in bonusblock"
                    << std::endl;
      }
    }
  }
  ...
  if (contents == CONTENT_LIGHT)
  {
    ...
    lightsprite =
        Surface::create("/images/objects/lightmap_light/bonusblock_light.png");
  }
...
BonusBlock::get_content_by_data(int d)
{
  switch (d)
  {
    ...
    case 6:
      ...
      SoundManager::current()->preload("sounds/switch.ogg");
      lightsprite = Surface::create(
          "/images/objects/lightmap_light/bonusblock_light.png");
      break;
    case 7:
      contents = CONTENT_TRAMPOLINE;
      // object = new Trampoline(get_pos(), false); //needed if this is to be
      // moved to custom
      break;
    case 8:
    ...
    case 9:
      contents = CONTENT_CUSTOM;
      object =
          std::make_shared<Rock>(get_pos(), "images/objects/rock/rock.sprite");
      break;
    case 10:
    ...
    case 12:
      contents = CONTENT_CUSTOM;
      object = std::make_shared<PowerUp>(
          get_pos(), "images/powerups/potions/red-potion.sprite");
      break;
    case 13:
...
BonusBlock::collision(GameObject& other, const CollisionHit& hit_)
{
  ...
  if (player)
  {
    if (player->does_buttjump) try_drop(player);
  }
...
BonusBlock::try_open(Player* player)
{
  ...
  if (player == NULL) player = sector->player;

  if (player == NULL) return;

  Direction direction =
      (player->get_bbox().get_middle().x > bbox.get_middle().x) ? LEFT : RIGHT;
  ...
  switch (contents)
  {
    case CONTENT_COIN:
    {
      Sector::current()->add_object(
          std::make_shared<BouncyCoin>(get_pos(), true));
      player->get_status()->add_coins(1);
      if (hit_counter != 0) Sector::current()->get_level()->stats.coins++;
      break;
    }
    ...
    {
      sector->add_object(
          std::make_shared<Star>(get_pos() + Vector(0, -32), direction));
    ...
    {
      break;
    }  // because scripts always run, this prevents default contents from being
       // assumed
    ...
    case CONTENT_TRAMPOLINE:
    {
      auto riser = std::make_shared<SpecialRiser>(
          get_pos(), std::make_shared<Trampoline>(get_pos(), false));
      sector->add_object(riser);
      SoundManager::current()->play("sounds/upgrade.wav");
      break;
    }
    case CONTENT_RAIN:
    {
      ...
      Sector::current()->add_object(
          std::make_shared<CoinRain>(get_pos(), true));
      SoundManager::current()->play("sounds/upgrade.wav");
      break;
    }
    case CONTENT_EXPLODE:
    {
      ...
      Sector::current()->add_object(
          std::make_shared<CoinExplode>(get_pos() + Vector(0, -40)));
      SoundManager::current()->play("sounds/upgrade.wav");
      break;
    }
  }
  ...
  if (script != "")
  {  // scripts always run if defined
    std::istringstream stream(script);
    Sector::current()->run_script(stream, "BonusBlockScript");
  }

  start_bounce(player);
  if (hit_counter <= 0 || contents == CONTENT_LIGHT)
  {  // use 0 to allow infinite hits
  }
...
BonusBlock::try_drop(Player* player)
{
  ...
  // First what's below the bonus block, if solid send it up anyway (excepting
  // doll)
  Rectf dest_;
  ...
  if (!Sector::current()->is_free_of_statics(dest_, this, true) &&
      !(contents == CONTENT_1UP))
  {
    try_open(player);
    return;
  ...
  switch (contents)
  {
    ...
    case CONTENT_FIREGROW:
    {
      drop_growup_bonus("images/powerups/fireflower/fireflower.sprite",
                        countdown);
      break;
    }
    ...
    case CONTENT_ICEGROW:
    {
      drop_growup_bonus("images/powerups/iceflower/iceflower.sprite",
                        countdown);
      break;
    }
    ...
    case CONTENT_AIRGROW:
    {
      drop_growup_bonus("images/powerups/airflower/airflower.sprite",
                        countdown);
      break;
    }
    ...
    case CONTENT_EARTHGROW:
    {
      drop_growup_bonus("images/powerups/earthflower/earthflower.sprite",
                        countdown);
      break;
    }
    ...
    case CONTENT_CUSTOM:
    {
      // NOTE: non-portable trampolines could be moved to CONTENT_CUSTOM, but
  ...
  if (countdown)
  {  // only decrease hit counter if try_open was not called
    if (hit_counter == 1)
    {
      sprite->set_action("empty");
    }
...
void
BonusBlock::raise_growup_bonus(Player* player, const BonusType& bonus,
                               const Direction& dir)
{
...
void
BonusBlock::drop_growup_bonus(const std::string& bonus_sprite_name,
                              bool& countdown)
{
  Sector::current()->add_object(
      std::make_shared<PowerUp>(get_pos() + Vector(0, 32), bonus_sprite_name));
  SoundManager::current()->play("sounds/upgrade.wav");
  countdown = true;
}
...
BonusBlock::draw(DrawingContext& context)
{
  ...
  {
    Vector pos =
        get_pos() + (bbox.get_size().as_vector() - lightsprite->get_size()) / 2;
    context.push_target();
...
{
  if (contentstring == "coin") return CONTENT_COIN;
  if (contentstring == "firegrow") return CONTENT_FIREGROW;
  if (contentstring == "icegrow") return CONTENT_ICEGROW;
  if (contentstring == "airgrow") return CONTENT_AIRGROW;
  if (contentstring == "earthgrow") return CONTENT_EARTHGROW;
  if (contentstring == "star") return CONTENT_STAR;
  if (contentstring == "1up") return CONTENT_1UP;
  if (contentstring == "custom") return CONTENT_CUSTOM;
  if (contentstring ==
      "script")  // use when bonusblock is to contain ONLY a script
    return CONTENT_SCRIPT;
  if (contentstring == "light") return CONTENT_LIGHT;
  if (contentstring == "trampoline") return CONTENT_TRAMPOLINE;
  if (contentstring == "rain") return CONTENT_RAIN;
  if (contentstring == "explode") return CONTENT_EXPLODE;
