...
BicyclePlatform::collision(GameObject& other, const CollisionHit&)
{
  ...
  if (pl)
  {
    ...
    if (contacts.insert(pomo).second)
      momentum += 0.1 * Sector::current()->get_gravity();
  }

  if (contacts.insert(&other).second)
    momentum += 0.1 * Sector::current()->get_gravity();
  return FORCE_MOVE;
}
...
BicyclePlatform::update(float elapsed_time)
{
  ...
  if (this == slave)
  {
    ...
    Vector dest_ = center + Vector(cosf(angle), sinf(angle)) * radius -
                   (bbox.get_size().as_vector() * 0.5);
  ...
  if (this == master)
  {
    ...
    angular_speed = std::min(
        std::max(angular_speed, static_cast<float>(-128 * M_PI * elapsed_time)),
        static_cast<float>(128 * M_PI * elapsed_time));
    Vector dest_ = center + Vector(cosf(angle), sinf(angle)) * radius -
                   (bbox.get_size().as_vector() * 0.5);
    movement = dest_ - get_pos();
