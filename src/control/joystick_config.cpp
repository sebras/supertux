...
{
  for (AxisMap::const_iterator i = joy_axis_map.begin();
       i != joy_axis_map.end(); ++i)
  {
    if (i->second == c) return i->first.second;
  }
...
{
  for (ButtonMap::const_iterator i = joy_button_map.begin();
...
{
  for (HatMap::const_iterator i = joy_hat_map.begin(); i != joy_hat_map.end();
       ++i)
  {
    if (i->second == c) return i->first.second;
  }
...
JoystickConfig::print_joystick_mappings() const
{
  ...
  for (AxisMap::const_iterator i = joy_axis_map.begin();
       i != joy_axis_map.end(); ++i)
  {
    std::cout << "Axis: " << i->first.second << " -> " << i->second
              << std::endl;
  }
  ...
  for (ButtonMap::const_iterator i = joy_button_map.begin();
       i != joy_button_map.end(); ++i)
  {
    std::cout << "Button: " << i->first.second << " -> " << i->second
              << std::endl;
  }
...
JoystickConfig::unbind_joystick_control(Controller::Control control)
{
  ...
  for (AxisMap::iterator i = joy_axis_map.begin(); i != joy_axis_map.end();
       /* no ++i */)
  {
    if (i->second == control)
  ...
  for (HatMap::iterator i = joy_hat_map.begin(); i != joy_hat_map.end();
       /* no ++i */)
  {
    if (i->second == control)
      joy_hat_map.erase(i++);
...
void
JoystickConfig::bind_joyaxis(JoyId joy_id, int axis,
...
void
JoystickConfig::bind_joybutton(JoyId joy_id, int button,
                               Controller::Control control)
{
  unbind_joystick_control(control);
...
JoystickConfig::read(const ReaderMapping& joystick_lisp)
{
  ...
  while (iter.next())
  {
    if (iter.get_key() == "map")
    {
      ...
      {
        log_info << "Invalid control '" << control << "' in buttonmap"
                 << std::endl;
      }
      else
      {
        ...
        {
          if (hat != SDL_HAT_UP && hat != SDL_HAT_DOWN && hat != SDL_HAT_LEFT &&
              hat != SDL_HAT_RIGHT)
          {
