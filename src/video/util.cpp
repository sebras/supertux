...
apply_pixel_aspect_ratio_pre(const Size& window_size, float pixel_aspect_ratio)
{
  if (true)
  {
    return Size(window_size.width * pixel_aspect_ratio, window_size.height);
  }
  else
  {
    return Size(window_size.width, window_size.height * pixel_aspect_ratio);
  }
}

inline void
apply_pixel_aspect_ratio_post(const Size& real_window_size,
                              const Size& window_size, float scale,
                              SDL_Rect& out_viewport, Vector& out_scale)
{
  Vector transform(
      static_cast<float>(real_window_size.width) / window_size.width,
...
calculate_scale(const Size& min_size, const Size& max_size,
                const Size& window_size, float magnification)
{
  ...
  if (scale == 0.0f)  // magic value
  {
    ...
    {
      scale =
          std::max(static_cast<float>(window_size.width) / max_size.width,
    ...
    {
      scale =
          std::min(static_cast<float>(window_size.width) / min_size.width,
...
calculate_viewport(const Size& max_size, const Size& window_size, float scale)
{
  ...
  viewport.w =
      std::min(window_size.width, static_cast<int>(scale * max_size.width));
  viewport.h =
      std::min(window_size.height, static_cast<int>(scale * max_size.height));
...
void
calculate_viewport(const Size& min_size, const Size& max_size,
                   ...
                   Size& out_logical_size, SDL_Rect& out_viewport)
{
  ...
  Size window_size =
      apply_pixel_aspect_ratio_pre(real_window_size, pixel_aspect_ratio);
...
float
calculate_pixel_aspect_ratio(const Size& source, const Size& target)
{
