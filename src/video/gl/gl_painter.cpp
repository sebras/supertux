...
{
inline void
intern_draw(float left, float top, float right, float bottom, float uv_left,
            float uv_top, float uv_right, float uv_bottom, float angle,
            float alpha, const Color& color, const Blend& blend,
            DrawingEffect effect)
{
  if (effect & HORIZONTAL_FLIP) std::swap(uv_left, uv_right);

  if (effect & VERTICAL_FLIP) std::swap(uv_top, uv_bottom);
  ...
  if (angle == 0.0f)
  {
    ...
    float uvs[] = {
        uv_left,  uv_top,    uv_right, uv_top,
        uv_right, uv_bottom, uv_left,  uv_bottom,
    };
  ...
  else
  {
    ...
    float vertices[] = {left * ca - top * sa + center_x,
                        left * sa + top * ca + center_y,
                        right * ca - top * sa + center_x,
                        right * sa + top * ca + center_y,
                        right * ca - bottom * sa + center_x,
                        right * sa + bottom * ca + center_y,
                        left * ca - bottom * sa + center_x,
...
GLPainter::draw_surface(const DrawingRequest& request)
{
  ...
  GLSurfaceData* surface_data =
      static_cast<GLSurfaceData*>(surface->get_surface_data());
  ...
  intern_draw(
      request.pos.x, request.pos.y, request.pos.x + surface->get_width(),
      request.pos.y + surface->get_height(), surface_data->get_uv_left(),
      surface_data->get_uv_top(), surface_data->get_uv_right(),
      surface_data->get_uv_bottom(), request.angle, request.alpha,
      request.color, request.blend, request.drawing_effect);
}
...
GLPainter::draw_surface_part(const DrawingRequest& request)
{
  ...
  std::shared_ptr<GLTexture> gltexture =
      std::dynamic_pointer_cast<GLTexture>(surface->get_texture());
  GLSurfaceData* surface_data =
      reinterpret_cast<GLSurfaceData*>(surface->get_surface_data());
  ...
  float uv_left =
      ...
      (uv_width * surfacepartrequest->srcrect.p1.x) / surface->get_width();
  float uv_top =
      surface_data->get_uv_top() +
      (uv_height * surfacepartrequest->srcrect.p1.y) / surface->get_height();
  float uv_right =
      surface_data->get_uv_left() +
      (uv_width * surfacepartrequest->srcrect.p2.x) / surface->get_width();
  float uv_bottom =
      surface_data->get_uv_top() +
      (uv_height * surfacepartrequest->srcrect.p2.y) / surface->get_height();
  ...
  intern_draw(request.pos.x, request.pos.y,
              ...
              request.pos.y + surfacepartrequest->dstsize.height, uv_left,
              uv_top, uv_right, uv_bottom, 0.0, request.alpha, request.color,
              Blend(), request.drawing_effect);
}
...
GLPainter::draw_gradient(const DrawingRequest& request)
{
  ...
  float vertices[] = {region.p1.x, region.p1.y, region.p2.x, region.p1.y,
                      region.p2.x, region.p2.y, region.p1.x, region.p2.y};
  glVertexPointer(2, GL_FLOAT, 0, vertices);
...
GLPainter::draw_filled_rect(const DrawingRequest& request)
{
  ...
  if (fillrectrequest->radius != 0.0f)
  {
    ...
    float radius = std::min(
        ...
        std::min(fillrectrequest->size.x / 2, fillrectrequest->size.y / 2));

    // inner rectangle
    Rectf irect(request.pos.x + radius, request.pos.y + radius,
...
{
  const InverseEllipseRequest* ellipse =
      static_cast<InverseEllipseRequest*>(request.request_data);

  glDisable(GL_TEXTURE_2D);
  glColor4f(ellipse->color.red, ellipse->color.green, ellipse->color.blue,
            ellipse->color.alpha);
  ...
  // Bottom
  vertices[p++] = SCREEN_WIDTH;
  vertices[p++] = SCREEN_HEIGHT;
  vertices[p++] = 0;
  vertices[p++] = SCREEN_HEIGHT;
  vertices[p++] = x;
  vertices[p++] = y + h;
