...
Tux::Tux(WorldMap* worldmap_)
    : back_direction(),
      ...
      sprite(SpriteManager::current()->create(
          "images/worldmap/common/tux.sprite")),
      controller(),
      input_direction(),
      direction(),
      tile_pos(),
...
{
  return ghost_mode || ((tile_data & Tile::WORLDMAP_NORTH && dir == D_NORTH) ||
...
Tux::tryContinueWalking(float elapsed_time)
{
  ...
  // stop if we reached a level, a WORLDMAP_STOP tile, a teleporter or a special
  // tile without a passive_message
  ...
  // if user wants to change direction, try changing, else guess the direction
