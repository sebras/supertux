...
{
  std::unique_ptr<ItemControlField> item(
      new ItemControlField(text, mapping, id));
  return add_item(std::move(item));
}
...
MenuItem*
Menu::add_string_select(int id, const std::string& text, size_t* selected,
                        std::vector<std::string> strings)
{
  std::unique_ptr<ItemStringSelect> item(
      new ItemStringSelect(text, strings, selected, id));
  return add_item(std::move(item));
}
...
Menu::process_input()
{
  ...
  {  // Scrolling
    ...
    pos.y = SCREEN_HEIGHT / 2 -
            scroll_offset * ((float(active_item) / (items.size() - 1)) - 0.5f) *
                2.0f;
  }
  ...
  if (controller->hold(Controller::UP) && menu_repeat_time != 0 &&
      real_time > menu_repeat_time)
  {
    menuaction = MENU_ACTION_UP;
  ...
  if (controller->hold(Controller::DOWN) && menu_repeat_time != 0 &&
      real_time > menu_repeat_time)
  {
    menuaction = MENU_ACTION_DOWN;
    menu_repeat_time = real_time + MENU_REPEAT_RATE;
  ...
  if (controller->hold(Controller::LEFT) && menu_repeat_time != 0 &&
      real_time > menu_repeat_time)
  {
    menuaction = MENU_ACTION_LEFT;
    menu_repeat_time = real_time + MENU_REPEAT_RATE;
  ...
  if (controller->hold(Controller::RIGHT) && menu_repeat_time != 0 &&
      real_time > menu_repeat_time)
  {
    menuaction = MENU_ACTION_RIGHT;
    menu_repeat_time = real_time + MENU_REPEAT_RATE;
...
Menu::draw_item(DrawingContext& context, int index)
{
  ...
  if (active_item == index)
  {
    ...
    context.draw_filled_rect(
        Rectf(Vector(pos.x - menu_width / 2 + 10 - 2, y_pos - 12 - 2),
              Vector(pos.x + menu_width / 2 - 10 + 2, y_pos + 12 + 2)),
        Color(1.0f, 1.0f, 1.0f, blink), 14.0f, LAYER_GUI - 10);
    context.draw_filled_rect(
        Rectf(Vector(pos.x - menu_width / 2 + 10, y_pos - 12),
...
Menu::draw(DrawingContext& context)
{
  ...
  {
    int text_width =
        (int)Resources::normal_font->get_text_width(items[active_item]->help);
    int text_height =
        (int)Resources::normal_font->get_text_height(items[active_item]->help);

    Rectf text_rect(
        pos.x - text_width / 2 - 8, SCREEN_HEIGHT - 48 - text_height / 2 - 4,
        pos.x + text_width / 2 + 8, SCREEN_HEIGHT - 48 + text_height / 2 + 4);

    context.draw_filled_rect(
        Rectf(text_rect.p1 - Vector(4, 4), text_rect.p2 + Vector(4, 4)),
        Color(0.2f, 0.3f, 0.4f, 0.8f), 16.0f, LAYER_GUI - 10);

    context.draw_filled_rect(text_rect, Color(0.6f, 0.7f, 0.8f, 0.5f), 16.0f,
                             LAYER_GUI - 10);
...
Menu::event(const SDL_Event& ev)
{
  ...
  switch (ev.type)
  {
    case SDL_MOUSEBUTTONDOWN:
      ...
      {
        Vector mouse_pos = VideoSystem::current()->get_renderer().to_logical(
            ev.motion.x, ev.motion.y);
        int x = int(mouse_pos.x);
        int y = int(mouse_pos.y);

        if (x > pos.x - get_width() / 2 && x < pos.x + get_width() / 2 &&
            y > pos.y - get_height() / 2 && y < pos.y + get_height() / 2)
        {
    ...
    case SDL_MOUSEMOTION:
    {
          ...
          y > pos.y - get_height() / 2 && y < pos.y + get_height() / 2)
      {
        ...
        if (!items[new_active_item]->skippable()) active_item = new_active_item;

        if (MouseCursor::current()) MouseCursor::current()->set_state(MC_LINK);
      }
      else
      {
