...
ItemControlField::ItemControlField(const std::string& text_,
                                   const std::string& input_, int _id)
    : MenuItem(text_, _id), input(input_)
{
}

void
ItemControlField::draw(DrawingContext& context, Vector pos, int menu_width,
                       bool active)
{
  context.draw_text(
      Resources::normal_font, input,
...
ItemControlField::get_width() const
{
  return Resources::normal_font->get_text_width(text) +
         Resources::normal_font->get_text_width(input) + 16;
}
