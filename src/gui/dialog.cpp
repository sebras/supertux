...
void
Dialog::add_default_button(const std::string& text,
...
void
Dialog::add_cancel_button(const std::string& text,
                          const std::function<void()>& callback)
{
  add_button(text, callback);
...
Dialog::process_input(const Controller& controller)
{
  ...
  if (m_cancel_button != -1 && (controller.pressed(Controller::ESCAPE) ||
...
{
  Rectf bg_rect(
      Vector(m_passive ? (SCREEN_WIDTH - m_text_size.width - 20)
                       : SCREEN_WIDTH / 2 - m_text_size.width / 2,
             m_passive ? (SCREEN_HEIGHT - m_text_size.height - 65)
                       : (SCREEN_HEIGHT / 2 - m_text_size.height / 2)),
      Sizef(m_text_size.width, m_text_size.height + 44));

  // draw background rect
  ctx.draw_filled_rect(bg_rect.grown(12.0f),
                       Color(0.2f, 0.3f, 0.4f, m_passive ? 0.3 : 0.8f), 16.0f,
                       LAYER_GUI - 10);

  ctx.draw_filled_rect(bg_rect.grown(8.0f),
                       Color(0.6f, 0.7f, 0.8f, m_passive ? 0.2 : 0.5f), 16.0f,
                       LAYER_GUI - 10);

  // draw text
  ctx.draw_text(Resources::normal_font, m_text,
                Vector(bg_rect.p1.x + bg_rect.get_width() / 2.0f, bg_rect.p1.y),
  ...
  for (int i = 0; i < static_cast<int>(m_buttons.size()); ++i)
  {
    ...
    if (i == m_selected_button)
    {
      float button_height = 24.0f;
      float blink =
          (sinf(real_time * M_PI * 1.0f) / 2.0f + 0.5f) * 0.5f + 0.25f;
